...
1. **PokeTrace `prices[source][tier]` exact key literals** — not confirmed by the OpenAPI spec (no enum, no examples). `PokeTraceProvider.ts` searches candidates defensively; spot-check against a real authenticated response.
2. **PokeTrace catalogue response field names** (`GET /cards`/`GET /sets`) — the spec confirms query params but is less explicit about some response field names; `PokeTraceCatalogueProvider.ts`'s `readField()` tries a short candidate list per field for the same reason.
