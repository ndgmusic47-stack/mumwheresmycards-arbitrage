 ...
 * site (opportunity engine, scan runner) goes through here, never the
...
export class MarketSnapshotCache {
  ...
  async getSnapshot(printing: CardPrinting): Promise<MarketSnapshotResult | null> {
    ...
    const fresh = await this.provider.getSnapshot(printing);
  ...
  private async persist(snapshot: MarketSnapshotResult): Promise<void> {
    await this.db.exec(
...
function rowToSnapshot(row: MarketSnapshotRow): MarketSnapshotResult {
  return {
    cardId: row.card_id,
